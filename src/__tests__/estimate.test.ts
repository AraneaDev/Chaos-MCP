...
/**
 * `cargo mutants --list` also accepts `--in-diff`, so a scoped estimate is
 * exact over the changed lines rather than approximate over the whole file.
 * Wired the same way the run path is: only `diffScope.kind === 'patch'` is
 * honoured (see `inDiffArgs` in engines/rust/args.ts).
 */
describe('estimateAudit --in-diff diff scoping', () => {
  it('passes --in-diff through to cargo mutants --list when diffScope is a patch', async () => {
    mockInvoke.mockResolvedValueOnce({
      stdout: 'src/lib.rs:1:1: replace foo -> bar\n',
      stderr: '',
    } as never);
  ...
  it('passes no --in-diff when diffScope is absent', async () => {
    mockInvoke.mockResolvedValueOnce({
      stdout: 'src/lib.rs:1:1: replace foo -> bar\n',
      stderr: '',
    } as never);
  ...
  it('ignores a non-patch diffScope on the estimate path too', async () => {
    mockInvoke.mockResolvedValueOnce({
      stdout: 'src/lib.rs:1:1: replace foo -> bar\n',
      stderr: '',
    } as never);
