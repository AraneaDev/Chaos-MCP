...
import type { PartialStrykerOptions, MutantResult, LogLevel } from '@stryker-mutator/api/core';
  ...
  it_heavy(
    ...
    async () => {
      ...
      const options: PartialStrykerOptions = {
        ...
        mutate: ['src/math.ts'],
        cleanTempDir: true,
        // `LogLevel` is a `const enum` in StrykerJS 10, so the bare string
        // literal no longer type-checks here. The runtime value is still
        // 'off' — the CLI path (`--logLevel off`) passes a string and is
        // unaffected.
      ...
      try {
        ...
        // StrykerJS 10 returns MutantResult[] (v9's `StrykerRunResult` wrapper
        // is gone, and `StrykerOptions` / `StrykerRunResult` are no longer
        // re-exported from core — hence the `@stryker-mutator/api/core` import
        // above). The assertions below read the JSON report rather than this
        // value, so the shape of the return is deliberately not relied upon.
        const result: MutantResult[] = await stryker.runMutationTest();
        void result;
      } finally {
