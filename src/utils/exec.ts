    ...
    execFile(
      ...
      (err, stdout, stderr) => {
        ...
        if (err) {
          ...
          // execFile sets err.status to the numeric exit code for non-zero
          // exits, err.code to 'ENOENT' when the binary is missing, and
          // err.signal when the child is killed by a signal.
          const result: ExecResult = {
            stdout: stdoutStr,
            stderr: stderrStr,
            exit: (errnoError as { status?: number }).status ?? null,
            signal: (errnoError as { signal?: NodeJS.Signals | null }).signal ?? null,
          };
