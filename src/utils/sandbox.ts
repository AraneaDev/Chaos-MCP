 ...
 * mutation runs).
 */
const SYMLINK_DIRS = ['node_modules', '.venv', 'venv', 'target'];
  ...
  try {
    ...
    cpSync(absoluteWorkspace, sandboxDir, {
      ...
      filter: (src: string) => {
        ...
        // Check user ignorePatterns against the full path and basename
        for (const pattern of userExcludes) {
          if (src.includes(pattern) || basename === pattern || src.endsWith(pattern)) {
            return false;
          }
        }
        return true;
