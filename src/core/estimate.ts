  ...
  if (fidelity === 'exact') {
    ...
    try {
      const res = await invokeMutationTool(
        ...
        // `estimate_audit` has no `diffBase` parameter (`core/tool-schema.ts`
        // declares only `filePath`/`withTiming`/`timeoutMs`), so there was never
        // a caller that could reach a scoped count here. `EstimateOptions` used
        // to carry a `diffScope` field wiring `--in-diff` into this argv anyway
        // (IMPORTANT finding: "an option with no producer"), removed rather
        // than wired, because wiring it properly means resolving a diff base and
        // materialising a patch the way `audit/diff-scope.ts` does, and this
        // module lives in `src/core`, which `knossos.json` forbids from
        // importing `audit` at all. Reachable only from a direct
        // `EstimateOptions` literal, entirely inside test files, is not "wired".
