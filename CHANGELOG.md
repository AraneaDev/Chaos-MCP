...
- **`stryker.config.mjs`** — tombstoned with `mutate: []` (explicit empty scope) and full resurrection steps A/B/C documented in the file header. Any direct invocation (`npx stryker run --configFile stryker.config.mjs`) now exits instantly with 0 mutants instead of falling back to the project's default mutate scope.
